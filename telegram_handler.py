...
class TelegramHandler:
    ...
    async def _handle_ath_response(self, text: str, source_bot_username: str):
        logger.debug(f"ATH_HANDLER ({source_bot_username}): Полный текст ответа:\n{text}")
        
        _ca_for_log = "CA_не_определен_до_паузы"
        if self.pause_event and self.pause_event.is_set():
        # Попытаемся извлечь CA хотя бы для лога, если возможно
            target_ca_temp = None
            potential_cas_log = []
            for match_log in AXIOM_CA_IN_URL_OR_BACKTICKS_REGEX.finditer(text):
                _ca = match_log.group(1) or match_log.group(2)
                if _ca and utils.is_valid_solana_address(_ca):
                    potential_cas_log.append(_ca)
                    if _ca in self.pending_ath_checks_ref:
                        target_ca_temp = _ca
                        break
            if not target_ca_temp and potential_cas_log:
                target_ca_temp = potential_cas_log[0] # Берем первый попавшийся для лога

            logger.info(f"ATH_HANDLER ({source_bot_username}): Бот на паузе. Ответ для CA '{target_ca_temp if target_ca_temp else 'не определен из текста'}' не будет добавлен в очередь ATH.")
            return # Не добавляем в очередь, если на паузе
        
        target_ca = None
        potential_cas_from_text = []
        for match in AXIOM_CA_IN_URL_OR_BACKTICKS_REGEX.finditer(text):
            ca_in_url = match.group(1)
            ca_in_backticks = match.group(2)
            if ca_in_url and utils.is_valid_solana_address(ca_in_url): potential_cas_from_text.append(ca_in_url)
            if ca_in_backticks and utils.is_valid_solana_address(ca_in_backticks): potential_cas_from_text.append(ca_in_backticks)
        
        if not potential_cas_from_text:
            logger.warning(f"ATH_HANDLER ({source_bot_username}): CA не найдены в тексте ответа.")
            return

        found_expected_ca = False
        for ca_candidate in potential_cas_from_text:
            if ca_candidate in self.pending_ath_checks_ref: # Проверяем по ссылке на словарь из main
                target_ca = ca_candidate
                found_expected_ca = True
                logger.info(f"ATH_HANDLER ({source_bot_username}): Ответ связан с ожидаемым CA '{target_ca}'.")
                break
        
        if not found_expected_ca:
            logger.warning(f"ATH_HANDLER ({source_bot_username}): Извлеченные CA ({potential_cas_from_text}) "
                           f"не найдены в ожидаемых ({list(self.pending_ath_checks_ref.keys())}). Игнор.")
            return

        match_ath = UNIVERSAL_ATH_REGEX.search(text)
        if match_ath:
            ath_str_val = (match_ath.group(1) or match_ath.group(2) or "").strip() # Берем непустую группу
            if ath_str_val:
                logger.info(f"ATH_HANDLER ({source_bot_username}): Для CA '{target_ca}' извлечена строка ATH: '{ath_str_val}'.")
                if ath_str_val.upper() == "N/A":
                    logger.info(f"ATH_HANDLER ({source_bot_username}): ATH для CA '{target_ca}' = 'N/A'.")
                    await self.ath_results_queue.put({'ca': target_ca, 'error': 'invalid_ath_format: N/A', 'source_bot': source_bot_username})
                else:
                    ...
                    if ath_seconds is not None:
                        ...
                        await self.ath_results_queue.put({'ca': target_ca, 'ath_seconds': ath_seconds, 'source_bot': source_bot_username})
                    else:
                        logger.warning(f"ATH_HANDLER ({source_bot_username}): Не удалось конвертировать ATH '{ath_str_val}' для CA '{target_ca}'.")
                        await self.ath_results_queue.put({'ca': target_ca, 'error': f'invalid_ath_format: {ath_str_val}', 'source_bot': source_bot_username})
            else:
                logger.warning(f"ATH_HANDLER ({source_bot_username}): ATH_REGEX сработал для CA '{target_ca}', но группы времени пусты.")
                await self.ath_results_queue.put({'ca': target_ca, 'error': 'empty_ath_time_string', 'source_bot': source_bot_username})
        else:
            logger.info(f"ATH_HANDLER ({source_bot_username}): ATH не найден в тексте для CA '{target_ca}'.")
            await self.ath_results_queue.put({'ca': target_ca, 'error': 'ath_info_not_found', 'source_bot': source_bot_username})
        pass

# telegram_handler.py, внутри класса TelegramHandler
    ...
    async def evaluate_ath_decision(self, ca: str, from_monitor: bool = False): # Добавлен флаг from_monitor
        ...
        if not from_monitor:
            ...
            if config.ATH_BOT_MODE_RUNTIME == "BOTH" and \
        ...
        if not timestamp_requested:
            ...
            should_check_ath2 = (config.ATH_BOT_MODE_RUNTIME == "BOTH" and 
        ...
        if not check_info.get('decision_made'): # Если еще не решили
            ...
            if config.ATH_BOT_MODE_RUNTIME == "BOTH" and config.BOT_ATH2_USERNAME and check_info.get('ath2_status') != 'not_sent':
