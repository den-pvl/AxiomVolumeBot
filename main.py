    ...
    for coin_data in parsed_coins:
        ...
        if ATH_BOT_MODE_RUNTIME in ["ATH1", "BOTH"]:
            if config.BOT_ATH1_USERNAME:
                ...
                if sent1:
                    if ca in pending_ath_checks:
                        pending_ath_checks[ca]['ath1_status'] = 'pending' # Обновляем статус
                    else:
                        logger.warning(f"RUN_CYCLE: CA {ca} отсутствует в pending_ath_checks при попытке обновить ath1_status.")
            ...
            if config.BOT_ATH2_USERNAME:
                ...
                if sent2:
                    if ca in pending_ath_checks:
                        ...
                        logger.warning(f"RUN_CYCLE: CA {ca} отсутствует в pending_ath_checks при попытке обновить ath2_status.")
                    sent_to_at_least_one_bot_in_this_iteration = True
                await asyncio.sleep(1.1)
            else:
...
async def process_ath_results():
    ...
    while not shutdown_event.is_set():
        got_item_from_queue = False # Флаг
        try:
            ...
            got_item_from_queue = True # Успешно получили элемент
            ...
            logger.error(f"ATH_PROCESSOR ({logger_name_ath}): Ошибка в цикле: {e}", exc_info=True)
            await asyncio.sleep(1)
        finally:
            if got_item_from_queue: # Вызываем task_done только если get() был успешен
                try:
                    telegram_handler.ath_results_queue.task_done()
                except ValueError:
                    logger.warning(f"ATH_PROCESSOR (finally): Попытка вызвать task_done() слишком много раз.")
                except Exception as e_td:
                    logger.error(f"ATH_PROCESSOR (finally): Неожиданная ошибка при task_done: {e_td}")
            await asyncio.sleep(0.5) #Небольшая задержка перед следующей попыткой в случае ошибки
...
async def main():
    ...
    telegram_handler = TelegramHandler(
        ...
        ath_bot_mode_runtime=ATH_BOT_MODE_RUNTIME
    )
