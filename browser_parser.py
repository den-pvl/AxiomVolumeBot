...
class BrowserParser:
    ...
    async def close_browser(self):
        ...
        logger.info("BROWSER_PARSER: Начало процедуры закрытия браузера...")

        if hasattr(self, 'page') and self.page and not self.page.is_closed():
            try:
                logger.debug("BROWSER_PARSER: Попытка закрыть страницу...")
                await self.page.close()
                logger.info("BROWSER_PARSER: Страница успешно закрыта.")
            except PlaywrightError as e:
                logger.warning(f"BROWSER_PARSER: Ошибка при закрытии страницы (возможно, уже закрыта): {e}")
            except Exception as e:
                logger.error(f"BROWSER_PARSER: Неожиданная ошибка при закрытии страницы: {e}", exc_info=True)
        self.page = None

        if hasattr(self, 'context') and self.context:
            # Контекст может быть уже закрыт, если браузер упал или был закрыт внешне
            # Проверка is_connected() для self.context.browser может быть ненадежной, если self.browser=None
            try:
                logger.debug("BROWSER_PARSER: Попытка закрыть контекст браузера...")
                await self.context.close()
                logger.info("BROWSER_PARSER: Контекст браузера успешно закрыт.")
            except PlaywrightError as e:
                if "closed" in str(e).lower() or "target" in str(e).lower():
                    logger.warning(f"BROWSER_PARSER: Ошибка при закрытии контекста (вероятно, уже закрыт или недоступен): {e}")
                else:
                    logger.error(f"BROWSER_PARSER: Ошибка Playwright при закрытии контекста: {e}", exc_info=True)
            except Exception as e:
                logger.error(f"BROWSER_PARSER: Неожиданная ошибка при закрытии контекста: {e}", exc_info=True)
        self.context = None
        self.browser = None # Браузер связан с контекстом, если контекст закрыт, браузер тоже

        if hasattr(self, 'playwright') and self.playwright:
            try:
                logger.debug("BROWSER_PARSER: Попытка остановить Playwright...")
                await self.playwright.stop()
                logger.info("BROWSER_PARSER: Playwright успешно остановлен.")
            except PlaywrightError as e:
                logger.error(f"BROWSER_PARSER: Ошибка Playwright при остановке: {e}", exc_info=True)
            except Exception as e:
                logger.error(f"BROWSER_PARSER: Неожиданная ошибка при остановке Playwright: {e}", exc_info=True)
        self.playwright = None
        
        logger.info("BROWSER_PARSER: Процедура закрытия браузера завершена.")
